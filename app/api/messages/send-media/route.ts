...
export async function POST(req: NextRequest) {
  let body: { number?: string; message?: string; attachmentUrl?: string };
  try {
    body = await req.json();
  } catch {
  ...
  const { number, message, attachmentUrl } = body;
  if (!number || !attachmentUrl) {
    return NextResponse.json(
      {
        error: "BAD_REQUEST",
        message: "Both `number` and `attachmentUrl` are required.",
      },
      { status: 400 },
    );
  }
  ...
  try {
    media = await MessageMedia.fromUrl(attachmentUrl, { unsafeMime: true });
  } catch (err) {
    return NextResponse.json(
      {
        error: "MEDIA_FETCH_FAILED",
        message: `Could not fetch attachment: ${(err as Error).message}`,
      },
      { status: 400 },
    );
  }
  ...
  try {
    const sent = await guard.client.sendMessage(resolved.chatId, media, {
